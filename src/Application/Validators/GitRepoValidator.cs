    ...
    public class GitRepoValidator : AbstractValidator<GitRepo>
    {
        public GitRepoValidator(IStringLocalizerFactory localizerFactory)
        {
            ...
            RuleFor(repo => repo.Name)
                .NotNull()
                .NotEmpty()
                .WithMessage(localizer[nameof(GitResource.NameIsRequired)]);

            RuleFor(repo => repo.Description)
                .NotNull()
                .NotEmpty()
                .WithMessage(localizer[nameof(GitResource.DescriptionIsRequired)]);

            RuleFor(repo => repo.Url)
                .NotNull()
                .NotEmpty()
                .WithMessage(localizer[nameof(GitResource.UrlIsRequired)]);
        }
    }
}