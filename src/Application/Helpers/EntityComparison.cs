namespace Stellantis.ProjectName.Application.Helpers
{
    public static class EntityComparison
    {
        ...
        /// <param name="originList">Objects Origin.</param>
        /// <param name="resultList">Objects Result.</param>
        /// <param name="propertyName">Property name to be compared (Id is compared if not set).</param>
        /// <returns></returns>
        /// <exception cref="InvalidOperationException"></exception>
        public static IEnumerable<int> PropsDiffer<TOrigin, TResult>(IEnumerable<TOrigin> originList, IEnumerable<TResult> resultList, string propertyName)
        {
            ArgumentNullException.ThrowIfNull(originList);
            ArgumentNullException.ThrowIfNull(resultList);
